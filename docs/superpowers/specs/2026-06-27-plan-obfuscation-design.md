...
- Tables temporaires utilisateur (`#temp` / `##temp`) : anonymisées. `tempdb` n'est plus whitelistée en bloc (seuls `Worktable`/`Workfile` par nom et les schémas système le restent). Un type de jeton dédié `NameKind.TempTable` produit `#Temp1`, `#Temp2`… (le `#` est conservé pour rester un identifiant temp valide). Le nom mutilé de l'arbre d'opérateurs (`#X____…hex`) et le nom propre du texte SQL (`#X`) sont corrélés via `NormalizeTempName` (suppression du suffixe `_{4,}[0-9A-Fa-f]*$`) et partagent un seul jeton. Couvre aussi `CREATE TABLE #X`. Vérifié sur corpus réel : 31/31 plans sans fuite de nom temp ni de nom structuré.
- Le repli regex de `StatementTextRewriter` utilise des bornes conscientes des identifiants (et non `\b`) pour les noms commençant par `#`/`@`/`$`, afin que tables temp et paramètres soient remplacés même dans les fragments non analysables (prédicats `ScalarString`, `StatementText` tronqué).
- Paramètres et variables locales (`@p`) : anonymisés en `@Param1`, `@Param2`… (le `@` fait partie du jeton, émis tel quel). Mappés à la volée dans les deux chemins du réécrivain (jetons `Variable` de ScriptDom dans le chemin principal ; pré-scan regex dans le repli) et dans l'arbre d'opérateurs (attribut `Column="@…"`). Un même `@p` partage un seul jeton entre arbre, texte SQL analysable et fragment non analysable. Les variables système `@@TRANCOUNT`/`@@ERROR`/`@@ROWCOUNT` sont préservées dans tous les chemins (texte et `Column=`). Vérifié sur corpus réel : 31/31 plans sans fuite de paramètre, aucun `@@` système perdu.
