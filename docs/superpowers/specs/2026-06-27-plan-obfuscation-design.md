...
- Limite restante : un nom de paramètre/variable (`@p`) qui n'apparaît que dans le texte SQL (signature de proc tronquée, `DECLARE` local) sans contrepartie dans l'arbre d'opérateurs n'est pas mappé. Les variables système `@@TRANCOUNT`/`@@ERROR` sont volontairement préservées. Décision de périmètre à trancher (cf. ci-dessous).
